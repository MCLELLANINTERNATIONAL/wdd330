...
/**
 * Fly-to-cart animation
 * Animates a clone of the source element to the cart icon
 */
function flyToCart(sourceElement) {
  ...
  const cartIcon = document.querySelector('.cart-btn') || document.querySelector('.cart-img');
  if (!cartIcon) {
    return; // Can't animate if cart icon doesn't exist
  }
